...
export function sanitizeSimpleInputs(raw: Partial<Record<keyof SimplePublicInputs, unknown>>): SimplePublicInputs {
  ...
  const out: SimplePublicInputs = {
    ...
    // The contract range, [currentAge, lifeExpectancy]. The stop age is the user's PLAN and the
    // projection engine accepts it as-is; it must never be silently rewritten to a different
    // plan. (A briefly-considered cap at 75 — the earliest-FI SEARCH ceiling — did exactly that
    // and was reverted: only the separate "tidligst" search is bounded at 75, and the Result
    // screen words its headline honestly when the earliest is unknowable instead. Phase 7 may
    // extend the engine's search window.)
